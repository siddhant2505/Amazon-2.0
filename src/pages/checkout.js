...
{/* Left hand section */}
            <div  className="flex-grow m-5 shadow-sm">
                ...
                <div className="flex flex-col p-5 space-y-10 bg-white">
                    ...
                    {items.map(({id,title,description,category,image,price,hasPrime,rating})=>(
        <CheckoutProduct key={id} title={title} description={description} price={price} category={category} image={image} rating={rating} hasPrime={hasPrime}/> 
      ))}
                {/* {items.map((item,i)=>{
